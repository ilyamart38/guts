...
urlpatterns = [
    ...
    url(r'^mgs/(?P<mgs_id>[0-9]+)/(?P<ms_id>[0-9]+)/new_campus/$',                          views.NewCampusInMs.as_view(),                name='new_campus_in_ms'),
    # адрес для добавления магистрали в МГС: /network/mgs/1/new_ms
    url(r'^mgs/(?P<mgs_id>[0-9]+)/new_ms/$',                                                views.NewMsInMgs.as_view(),         name='new_ms_in_mgs'),
    ...
    url(r'^campus/(?P<campus_id>[0-9]+)/new_thread/$',                                      views.NewThreadInCampus.as_view(),   name='new_thread_in_campus'),
    ...
    url(r'^thread/(?P<thread_id>[0-9]+)/new_net/$',                                         views.SubnetInThread.as_view(),               name='new_net_in_thread'),
    ...
    url(r'^thread/(?P<thread_id>[0-9]+)/new_access_node/$',                                 views.NewAccessNodeInThread.as_view(),  name='new_access_node_in_thread'),
    ...
    url(r'^access_node/(?P<access_node_id>[0-9]+)/new_access_switch/$',                     views.NewAccessSwitchInNode.as_view(),            name='new_access_switch_in_node'),
