...
def analiz_switch(switch):
    # вычисляем с какой МГС этот коммутатор
    if len(switch['group'].split(' -> ')) > 3:
        """
        1. Вычисляем номер МГС к которой пренадлежит коммутатор
        ...
        if switch['mgs'] == '':
            ...
            sys.exit()
        #print(switch['mgs'])
        # проверяем есть ли мгс с номером, определенным из группы коммутатора
        mgs_num = int(switch['mgs'])
        if not MGS.objects.filter(mgs_num=mgs_num):
            print('%s - NE OK!!!' % switch['mgs'])
        ...
        """
        2. из ip адреса коммутаторов вычисляем номер магистрали и добавляем недостающие в базу
        """
        if switch['ip']:
            ip = switch['ip']
            if check_ip(ip):
                ...
                if komm_id > 100000:
                    ...
                    if not ACCESS_SWITCH.objects.filter(ip=switch['ip']):
                        if mgs != mgs_num:
                            print('<%s> %s != %s, %s' % (ip, mgs, mgs_num, switch['group']))
                        if MGS.objects.filter(mgs_num = mgs):
                            sw_mgs = MGS.objects.get(mgs_num = mgs)
                            # Проверяем есть ли МС соответствующая ip-адресу
                            if MS.objects.filter(mgs=sw_mgs, num_in_mgs=ms):
                                ...
                                #Если ip соответствует несуществующей МС, создаем новую МС, соответствующую 
                                #ip-адресу коммутатора
                            ...
                            #Если ip соответствует несуществующей МГС, создаем новую МГС, соответствующую 
                            #ip-адресу коммутатора
                            print('%s - NE OK!!!' % mgs)
                            # Создаем недостающую МГС
                            new_mgs = MGS(
        ...
        print('Мало полей в описании группы: <%s>' % switch['group'])
        sys.exit()
    #return
